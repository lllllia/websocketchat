...
// 避免用户在当前页面刷新
if (user.name) (window as any).socket.emit("register", user.name);

onBeforeUnmount(() => {
    // 用户离开
    (window as any).socket.emit('leave', user)
});
...
const sendAction = () => {
    ...
    (window as any).socket.emit('chat', info);
}
...
const onFileChange = async (uploadFile: UploadFile) => {
    ...
    if (data) {
        ...
        (window as any).socket.emit('avatar', user);
    }
}
